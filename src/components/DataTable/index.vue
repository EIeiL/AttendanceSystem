<template>
  <div class="attendance-table">
    <!-- 表格内容 -->
    <el-table class="mytable" :data="tableData" style="width: 90%" :default-sort="{ prop: 'date', order: 'descending' }"
      :header-cell-style="tableHeaderColor" border>
      <el-table-column v-for="(item, index) in columnOptions" :key="index" fixed="right"
        :prop="item.operate ? '' : item.prop" :label="item.label" :sortable="item.sortable">
        <template slot-scope="scope">
          <div v-if="
              item.operate.includes('edit') || item.operate.includes('delete')
            ">
            <el-button @click="onEdit(scope.row)" type="text" size="small" v-if="item.operate.includes('edit')">
              编辑</el-button>
            <el-button @click="onDelete(scope.row)" type="text" size="small" class="delColor" v-if="
