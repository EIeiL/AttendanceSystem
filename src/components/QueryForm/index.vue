<template>
  <el-form ref="formRef" :inline="true" :model="formInline" class="demo-form-inline form-padding attendance-form">
    <!-- 表单内容 -->
    <span v-for="(item, index) in formOptions" :key="index">
      <el-form-item :key="item.prop" :prop="item.prop" :label="item.label ? item.label + '：' : ''" :rules="item.rules">
    ...
    <div class="table-tools">
      <el-form-item v-for="(item, index) in btnTools" :key="index">
        <el-button type="primary" class="system-bgcolor" @click="onSubmit(item.prop)" :prop="item.prop">{{ item.text }}
        </el-button>
      </el-form-item>
    </div>
  </el-form>
